...
var wCalendar = function() {
	...
	var init = function(ele, callback) {
		...
		var calendarMode, calendarEle, calendarX, calendarY;
		...
		var _showCalendar = function() {
			...
			var top, left, calendarBox = calendarEle.find('.wCalendar_box'), offsetX = ele.offset().left, offsetY = ele.offset().top, width = ele.width(), height = ele.height();
			...
			if (jQuery(window).width() > 2 * calendarX) {
				left = offsetX;
			...
			calendarBox.css({'top': top + 'px', 'left': left + 'px'});
		...
		var _bindInitEvents = function() {
			...
			ele.click(function(e) {
				...
				calendarX = e.clientX || e.originalEvent.x;
